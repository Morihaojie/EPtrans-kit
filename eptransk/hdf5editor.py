...
def write_dynamical_matrix_sparse_data_to_hdf5(self,hdf5filename,input_symmetrized_numpy_matrix):
    indice,row_start,shape,value = self.change_symmetrized_numpy_matrix_to_dynamical_matrix_sparse_data(input_symmetrized_numpy_matrix)
    filestr = '/DynamicalMatrix_0/dynamical_matrix/matrix_vector/0/'
...
def write_sparse_data_to_hdf5(self,read_filestr,read_hdf5filename,write_filestr,write_hdf5filename):
    indice,row_start,shape,value = self.trans_new_numpy_data_to_numpy_data_by_filestr(read_filestr,read_hdf5filename)
...
def gen_hdf5_file_name_by_fix_atoms_list(prefix,fix_atoms_list):
...
def gen_hdf5_file_name_by_replaced_atoms_list(prefix,replaced_atoms_list):
    hdf5filename = ''+str(prefix)+'_'
...
def find_translation_vectors(repeat_times):
    ni = repeat_times[0]
    nj = repeat_times[1]
    nk = repeat_times[2]
    translation_vectors=[]

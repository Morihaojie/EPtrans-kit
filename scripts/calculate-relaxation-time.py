...
def calculate_energy_dependent_relaxation_time(read_hdf5filename):
    ...
    scattering_rates_total = np.sum(completed_inverse_relaxation_time[0], axis=0)   # 单位 1/s
    relaxation_time_total = np.where(scattering_rates_total > 0, 1.0 / scattering_rates_total, np.nan)
    ...
    np.savetxt(output_path,
               ...
               header='Energy - E_F (eV)   Energy_Dependent_Relaxation_Time (s)')
    print("File 'energy_dependent_relaxation_time.dat' written.")
...
def calculate_carrier_average_relaxation_time(read_hdf5filename, T, kweights=None):
    ...
    fermi_level = FC.read_hdf5('/Mobility_0/fermi_level/array/data', read_hdf5filename)
    scattering_rates_total = np.sum(completed_inverse_relaxation_time[0], axis=0)   # 单位 1/s
    relaxation_time_total = np.where(scattering_rates_total > 0, 1.0 / scattering_rates_total, np.nan)   # 单位 s
    ...
    def average_relaxation_time_for_mask(mask):
        tau_masked = relaxation_time_total[mask]
        w_masked = weight[mask]
        finite = np.isfinite(tau_masked)
        if not np.any(finite):
            return np.nan
        tau_finite = tau_masked[finite]
        w_finite = w_masked[finite]
        sum_w = np.sum(w_finite)
        if sum_w == 0:
            return np.nan
        avg_tau = np.sum(tau_finite * w_finite) / sum_w
        return avg_tau
